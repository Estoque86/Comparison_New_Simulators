...
class SIM_API cTopology : public cOwnedObject
{
  public:
    ...
    class SIM_API Node
    {
      ...
      private:
        ...
        double dist;
        Link *out_path;
	std::vector<Link *> out_paths;

	// variables used by the betweenness centrality algorithm [brandes01jms]
	double sigma;
	double btw;
	double delta;
      ...
      public:
        ...
        int getNumPaths() const  {return out_paths.size();}
         ...
         * length.)
         */
        LinkOut *getPath() const  {return (LinkOut *)out_path;}
        LinkOut *getPath(int i) const  {return (LinkOut *)out_paths[i];}
        //@}
	
	double get_betweenness() {return btw;}
    };
  ...
  public:
    ...
    void weightedSingleShortestPathsTo(Node *target);
    void weightedMultiShortestPathsTo(Node *target);
    void extractWeight(std::string file);
    /**
