    ...
    echo $realReq
     		#realReq=$simDuration*$z
      		...
      		do
         		...
         		/usr/bin/time -f "\n%E \t elapsed real time \n%U \t total CPU seconds used (user mode) \n%S \t total CPU seconds used by the system on behalf of the process \n%M \t memory (max resident set size) [kBytes] \n%x \t exit status" -o ${infoDir}/Info_SIM\=${sim}_T\=${T}_REQ\=${req}_M\=${M}_C\=${C}_L\=${z}_A\=${k}_R\=${i}.txt $main --run "ndn-grid100 --catalogCardinality\=${M} --numReqTot\=${req} --cacheToCatalogRatio\=${C} --lambda\=${z} --alpha\=${k} --plateau\=${plateau} --simType\=${T} --simDuration\=${simDuration} --RngSeed\=1 --RngRun\=${j}" > $logDir/stdout/SIM\=${sim}_T\=${T}_REQ\=${req}_M\=${M}_C\=${C}_L\=${z}_A\=${k}_R\=${i}.out 2>&1   
